...
//MessageToast.show("data not loaded successfully");
     ...
     this.getView().byId("idImageProduct").setSrc(sImagePath);
        ...
        onGetHighestPriceProduct: function(){
            debugger;
            //1.Read Category from input screen
            var oinput = this.getView().byId("idProductCategory").getValue();
            //2.Get OData Model
            var oDataModel = this.getView().getModel();
            //3.Get JSON Data model
            var oJsonModel = this.getView().getModel("prod");
            //4.Call OData functions import
            oDataModel.callFunction("/GetHighestPriceProduct",{
                 //4.1 Pass Parameter Category

                 urlParameters: {
                    I_CATEGORY: oinput
                 },
                 success: function(data) {
                     //4.2 Bind data to view
                    oJsonModel.setProperty("/ProductSet",data);
                    MessageBox.success("Data loaded successfully"); 
